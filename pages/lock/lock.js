...
Page({
  data: {
    ...
    read_uuid: '0000FEC9-0000-1000-8000-00805F9B34FB',
    read_data: false,//读
    readIndicate_uuid: '0000FEC8-0000-1000-8000-00805F9B34FB',
    readIndicate_data: false,//读指令
            ...
            wx.startBluetoothDevicesDiscovery({
              ...
              success: function (res) {
                ...
                wx.onBluetoothDeviceFound(function (devices) {
                  ...
                  if (devices.devices[0].name == 'NokeLock') {
                    that.setData({
                      button_text: '断开连接',
                      button_type: 'warn',
  ...
  select_characteristics: function () {
    var that = this;
    var services_list = that.data.devices_data_services;
    var read = { service: false, characteristic: false };
    var write = { service: false, characteristic: false };
    var readIndicate = { service: false, characteristic: false };
    ...
    services_list.forEach(function (value, index, array) {
      array[index].characteristics.forEach(function (values, indexs, arrays) {
        if (values.uuid == that.data.read_uuid) {
          console.log('找到读取控制特征值', values);
          read.service = value;
        ...
        if (values.uuid == that.data.write_uuid) {
          console.log('找到写控制特征值', values);
          write.service = value;
        ...
        if (values.uuid == that.data.readIndicate_uuid) {
          console.log('找到读指令控制特征值', values);
          readIndicate.service = value;
          readIndicate.characteristic = values;
          that.setData({
            readIndicate_data: readIndicate
          });
        }
  ...
  read_control: function () {
    var that = this;
    var read_data = that.data.read_data;
    wx.readBLECharacteristicValue({
      deviceId: that.data.devices_data.deviceId,
      serviceId: read_data.service.uuid,
      characteristicId: read_data.characteristic.uuid,
      success: function (res) {
        console.log('读取设备锁信息', res);
        that.setData({
          steps_data_count: "..."
        });
      }
  ...
  write_control: function (attr) {
    var that = this;
    var write_data = that.data.write_data;
    let buffer = new ArrayBuffer(1);
    let dataView = new DataView(buffer);
    //dataView.setUint8(0, attr.currentTarget.dataset.strength);
    dataView.setUint8(0, '28BF68EB5D3334C8613762E87706F9FE');
    wx.writeBLECharacteristicValue({
      deviceId: that.data.devices_data.deviceId,
      serviceId: write_data.service.uuid,
      characteristicId: write_data.characteristic.uuid,
      value: buffer,
      success: function (res) {
        console.log('写命令发送成功', res);
      }
    });
  },
